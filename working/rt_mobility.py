...
def compute_rt(ts, mobility_data, r0, r1, alpha, gamma):    
    rt_u = r0 * gamma * mobility_data[ts] +  r1 * (1 - gamma * mobility_data[ts])
    rt = np.dot(rt_u, alpha)
    return rt

def compute_rt_samples(samples, mobility_data, N, country_ix):
...
#         mu[name], pi[name] = compute_mu_pi_3(corr_samples[name])
    ...
    for i in range(corr_samples['r0'].shape[0]):
        rt_pi[i] = compute_rt(
            ...
            corr_samples.get(f'gamma_{country_ix}', np.ones(corr_samples['r0'].shape))[i]
        )
